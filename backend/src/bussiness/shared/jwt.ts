...
export default {
    ...
    createToken: (data: any , role: string, expireIn: string): string => {
        try {

            const tokenex: any = process.env.USER_TOKEN_EXP ? process.env.USER_TOKEN_EXP : 7;
            const tokenExpiry: any = (Math.floor(Date.now() / 1000) + 60 * 60 * 24 * tokenex);
